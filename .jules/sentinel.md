...
## 2026-04-21 - Secure URL Launching Helper\n **Vulnerability:** Unsafe URL Launching via  allows execution of potentially dangerous schemes (e.g., , , ).\n **Learning:** Using  with raw strings directly exposes applications to unauthorized protocol executions and silent failures if URLs cannot be handled.\n **Prevention:** Created a centralized helper  in  to strictly validate parsed Uri schemes (allowing only , , ), verify handling via , and provide user feedback via  for failed or unsafe attempts. Always replace inline  calls with this utility.
